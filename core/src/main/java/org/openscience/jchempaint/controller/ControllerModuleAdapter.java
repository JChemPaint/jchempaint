...
public abstract class ControllerModuleAdapter implements IControllerModule {
     ...
     *
     * @param worldCoord
     * @return a AtomBondSet containing the atoms/bond that should be affected
     * by this action. Otherwise <code>null</code>.
     */
    protected AtomBondSet getSelectAtomBondSet(Point2d worldCoord) {
        ...
        for (IAtom a : localSelection.elements(IAtom.class))
            atomBondSet.add(a);
        for (IBond b : localSelection.elements(IBond.class))
            atomBondSet.add(b);
        return atomBondSet;
    }
